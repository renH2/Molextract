...
class sac:
    ...
    def __init__(self, writer, args, env_fn, actor_critic=core.GCNActorCritic, ac_kwargs=dict(), seed=0,
                 steps_per_epoch=4000, epochs=100, replay_size=int(1e6), beta=0.99,
                 ...
                 max_ep_len=1000, save_freq=1, train_alpha=True):
        super().__init__()
        ...
        self.writer = writer
        self.scaffold_idx = args.scaffold_idx
        self.root = f'gen/{args.name_full}'
        self.fname = f'gen/{args.name_full}/action_{args.max_action}_scaffold_{args.scaffold_idx}.csv'
        self.args = args
        ...
        ## OPTION2: OPTIMIZER SETTING need to modify
    ...
    def compute_loss_q(self, data):
        ...
        with torch.no_grad():
            ...
            backup = r + self.beta * (1 - d) * q_pi_targ
    ...
    def train(self):
        ...
        for t in tqdm(range(total_steps)):
            ...
            if (t % self.save_freq == 0):
                ...
                content = sorted(self.bank, key=lambda x: (x[0], x[1]), reverse=True)[:100]
                content = self.env.predictor.top_record
                extraction_result, _, _ = calculate_extraction(content, self.scaffold_idx)
                if not os.path.exists(self.root):
                    os.makedirs(self.root)
                self.fname = f'gen/{self.args.name_full}/action_{self.args.max_action}_scaffold_{self.args.scaffold_idx}_epoch{t}.csv'
                with open(self.fname, 'w') as f:
                    for i in range(len(content)):
                        str1 = f'{Chem.MolToSmiles(content[i][2])}' + '\n'
                        f.write(str1)
                fcd_result = calculate_fcd(content, self.scaffold_idx)
                print('extraction result:', extraction_result)
                print('fcd result:', fcd_result)
...
def calculate_fcd(records, scaffold_idx):
    if len(records) == 0:
        return 0
    scaffold_smi = SCAFFOLD_VOCAB[scaffold_idx]
